...
extern int lmpio_read(const char *filename, double **positions, int **types,
  double **masses, double **size, char **atom_style, int *numatoms,
  int *numtypes)
{
  ...
  // atom_style
  // AAaaand the LAMMPS team changed the
//  lammps.atom->create_avec(style, 0, NULL, lammps.suffix);
  lammps.atom->create_avec(style, 0, NULL, 1);
  ...
  if (positions != NULL)
  {
    ...
    *positions = reinterpret_cast<double*>(malloc(
      (*numatoms) * sizeof(double) * 3));
...
extern int lmpio_write(const char *filename, int numatoms, int numtypes,
  double *positions, int *types, double *masses, double *size,
  char **atom_style)
{
